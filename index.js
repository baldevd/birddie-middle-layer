...
app.get('/bird_search', (req, res, next) => {
    connectionPool.getConnection(function (err, connection) {
        ...
        connection.query({
            sql: 'SELECT Prey_Common_Name, Fraction_Diet, Diet_Type FROM `birddietdb` WHERE Common_Name = ?',
            timeout: 10000,
...
app.get('/advanced_bird_search', (req, res, next) => {
    connectionPool.getConnection(function (err, connection) {
        ...
        connection.query({
            sql: 'SELECT Prey_Common_Name, Fraction_Diet, Diet_Type FROM `birddietdb` WHERE Common_Name = ? AND Observation_Year_Begin >= ? AND Observation_Year_End <= ?',
            timeout: 10000,
...
app.get('/more_advanced_bird_search', (req, res, next) => {
    connectionPool.getConnection(function (err, connection) {
        ...
        connection.query({
            sql: 'SELECT Prey_Common_Name, Fraction_Diet, Diet_Type FROM `birddietdb` WHERE Common_Name = ? AND Observation_Year_Begin >= ? AND Observation_Year_End <= ? AND Location_Region = ?, AND Observation_Season = ?',
            timeout: 10000,
